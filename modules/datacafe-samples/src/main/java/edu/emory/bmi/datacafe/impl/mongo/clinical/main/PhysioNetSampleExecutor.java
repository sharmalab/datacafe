...
public class PhysioNetSampleExecutor {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i <  collections.length; i++) {
            idsArray[i] = mongoConnector.getIDs(database, collections[i], documents[i]);
        }

        if(logger.isDebugEnabled()) {
            logger.debug("The list of IDs retrieved");
        }
        ...
        for (int i = 0; i < collections.length; i++) {
            chosenAttributes[i] =
                    mongoConnector.getAllAttributeValuesExceptAutoGenMongoId(database, collections[i], idsArray[i]);
        }
